...
/**
 * Integration tests for {@link ApplicationController}.
 * Covers job application lifecycle including applying, retrieving, and updating application status.
 */
@SpringBootTest
...
class ApplicationControllerTest {
    ...
    void applyForJobShouldReturnCreatedForApplicant() throws Exception {
        ...
        when(applicationService.applyForJob("user@local.test", 1L)).thenReturn(application);
    ...
    void applyForJobShouldReturnNotFoundWhenJobDoesNotExist() throws Exception {
        when(applicationService.applyForJob("user@local.test", 404L))
    ...
    void getApplicationsShouldRequireAuthentication() throws Exception {
        when(applicationService.getApplicationsByEmail("user@local.test")).thenReturn(List.of(new Application()));
    ...
    void getApplicationByIdShouldReturnForbiddenWhenApplicantDoesNotOwnApplication() throws Exception {
        ...
        owner.setEmail("other-user@local.test");
        Job job = new Job("Java Developer", "Build APIs", "ACME");
